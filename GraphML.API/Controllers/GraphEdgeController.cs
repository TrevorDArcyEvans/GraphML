...
namespace GraphML.API.Controllers
{
  ...
  public sealed class GraphEdgeController : OwnedGraphMLController<GraphEdge>
  {
    ...
    [ProducesResponseType(statusCode: (int) HttpStatusCode.OK, type: typeof(IEnumerable<GraphEdge>))]
    [ProducesResponseType(statusCode: (int) HttpStatusCode.NotFound)]
    public ActionResult<IEnumerable<GraphEdge>> AddByFilter([FromRoute] [Required] Guid graphId, [FromBody] string filter)
    {
      var retval = _graphEdgeLogic.AddByFilter(graphId, filter);
      return Ok(retval);
    }
